...
g1 <- plotExpression(fds, result=gr1) + 
...
yticks <- c(30, 50, 100)
g3 <- plotExpression(fds, result=gr2, deltaPsiCutoff=0.1) + 
    scale_y_log10(breaks=yticks + 1, labels=yticks) + 
    scale_x_log10(breaks=xticks + 1, labels=xticks) + 
...
g4 <- plotQQ(fds, result=gr2, deltaPsiCutoff=0.1)
g4
