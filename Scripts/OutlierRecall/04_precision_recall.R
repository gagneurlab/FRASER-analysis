...
#'  input:
#'   - fitHtml: 'Output/html/OutlierInjection/{dataset}/{psiType}/{delta}/{method}_pvalues.html'
#'  output:
#'   - plotData: '`sm expand(config["DATADIR"] + "/processedData/precRec/{{dataset}}/inject_{{delta}}/{{psiType}}/{{method}}_plotData_{{outlierType}}_{index}.tsv.gz", index=indices)`'
#'   - wBhtml: 'Output/html/OutlierRecall/{dataset}/{delta}/{psiType}/{method}_precRec_{outlierType}.html'
#'  type: noindex
#'---
